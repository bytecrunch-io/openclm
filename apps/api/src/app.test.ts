...
describe('contracts API vertical slice', () => {
  ...
  it('uses scoped client credentials for signing handoffs and generic condition decisions', async () => {
    const app = await testApp();
    ...
    const credentials = await integration.json() as { integration: { clientId: string }; clientSecret: string };
    const listed = await (await app.request('/v1/integrations')).json() as unknown; expect(JSON.stringify(listed)).not.toContain(credentials.clientSecret);
    const invalidToken = await app.request('/oauth/token', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded', authorization: `Basic ${Buffer.from(`${credentials.integration.clientId}:wrong`).toString('base64')}` }, body: 'grant_type=client_credentials' }); expect(invalidToken.status).toBe(401);
    const tokenResponse = await app.request('/oauth/token', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded', authorization: `Basic ${Buffer.from(`${credentials.integration.clientId}:${credentials.clientSecret}`).toString('base64')}` }, body: 'grant_type=client_credentials&scope=conditions%3Aread%20signing_sessions%3Awrite' });
    expect(tokenResponse.status).toBe(200); const access = await tokenResponse.json() as { access_token: string; expires_in: number }; expect(access.expires_in).toBe(300); const machineHeaders = { 'content-type': 'application/json', authorization: `Bearer ${access.access_token}` };
    const sessionResponse = await app.request('/integration/v1/signing-sessions', { method: 'POST', headers: machineHeaders, body: JSON.stringify({ integrationKey: 'customer-portal', subject: 'portal-user-42', email: 'visitor@example.com', displayName: 'Visitor', templateKey: 'mutual-nda', returnUrl: 'https://portal.example/workflows', metadata: { workflow: 'supplier-onboarding' } }) });
    ...
    expect(completed).toMatchObject({ agreement: { status: 'executed' }, participant: { signature: { provider: 'development_witness', providerSignatureId: expect.stringMatching(/^devsig_/), authenticationMethod: 'integration_handoff', authenticationIssuer: expect.stringMatching(/^urn:bytecrunch:integration:/), authenticationSubject: 'portal-user-42', consentVersion: '2026-08-30' } } });
    ...
    const evaluation = await app.request('/integration/v1/conditions/evaluate', { method: 'POST', headers: machineHeaders, body: JSON.stringify({ integrationKey: 'customer-portal', subject: 'portal-user-42', operator: 'all', conditions: [{ kind: 'subject_signed', templateKey: 'mutual-nda', minimumVersion: 1 }, { kind: 'agreement_executed', templateKey: 'mutual-nda', minimumVersion: 1 }] }) });
    expect(evaluation.headers.get('cache-control')).toBe('no-store'); expect(await evaluation.json()).toMatchObject({ decisionId: expect.stringMatching(/^decision_/), integrationKey: 'customer-portal', issuer: expect.stringMatching(/^urn:bytecrunch:integration:/), subject: 'portal-user-42', met: true, operator: 'all', conditions: [{ kind: 'subject_signed', met: true }, { kind: 'agreement_executed', met: true }] });
    const unknown = await app.request('/integration/v1/conditions/evaluate', { method: 'POST', headers: machineHeaders, body: JSON.stringify({ integrationKey: 'customer-portal', subject: 'unknown-subject', conditions: [{ kind: 'agreement_executed', templateKey: 'mutual-nda' }] }) }); expect(await unknown.json()).toMatchObject({ met: false, conditions: [{ met: false }] });
    expect((await app.request('/integration/v1/conditions/evaluate', { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' })).status).toBe(401);
  ...
  it('requires the configured participant OIDC provider before accepting a shared-identity handoff', async () => {
    const repository = new MemoryRepository(); await repository.init(); const app = createApp(repository);
    const issuer = 'https://identity.example.com'; const clientId = 'participant-contracts'; const clientSecret = 'participant-client-secret';
    const configured = await app.request('/v1/plugin-installations/participant-oidc', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ enabled: true, configuration: { issuerUrl: issuer, authorizationEndpoint: `${issuer}/authorize`, tokenEndpoint: `${issuer}/token`, jwksUri: `${issuer}/jwks`, clientId, clientSecret } }) }); expect(configured.status).toBe(200);
    const createdResponse = await app.request('/v1/integrations', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ key: 'fiftysixty', name: 'FiftySixty website', mappingStrategy: 'shared_oidc', identityProviderKey: 'participant-oidc', allowedRedirectUris: ['https://fiftysixty.com/projects'] }) });
    const created = await createdResponse.json() as { integration: { clientId: string }; clientSecret: string };
    const oauth = await app.request('/oauth/token', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded', authorization: `Basic ${Buffer.from(`${created.integration.clientId}:${created.clientSecret}`).toString('base64')}` }, body: 'grant_type=client_credentials' }); const accessToken = (await oauth.json() as { access_token: string }).access_token;
    const sessionResponse = await app.request('/integration/v1/signing-sessions', { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${accessToken}` }, body: JSON.stringify({ integrationKey: 'fiftysixty', subject: 'cognito-sub-123', email: 'investor@example.com', displayName: 'Investor', templateKey: 'mutual-nda', returnUrl: 'https://fiftysixty.com/projects' }) }); expect(sessionResponse.status).toBe(201);
    const handoff = await sessionResponse.json() as { handoffUrl: string }; const integrationToken = new URL(handoff.handoffUrl).searchParams.get('integrationToken')!;
    const exchange = await app.request('/public/integration-sessions/exchange', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ token: integrationToken }) });
    expect(exchange.status).toBe(202); expect(exchange.headers.get('set-cookie')).toBeNull(); const authRequired = await exchange.json() as { authenticationUrl: string }; expect(authRequired.authenticationUrl).toContain('/auth/participant/bytecrunch');

    const { privateKey, publicKey } = await generateKeyPair('RS256'); const jwk = { ...await exportJWK(publicKey), kid: 'participant-test-key', alg: 'RS256', use: 'sig' };
    let idToken = ''; const fetchMock = vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => { const url = String(input); if (url.endsWith('/.well-known/openid-configuration')) return Response.json({ issuer, authorization_endpoint: `${issuer}/authorize`, token_endpoint: `${issuer}/token`, jwks_uri: `${issuer}/jwks` }); if (url.endsWith('/token')) return Response.json({ id_token: idToken }); if (url.endsWith('/jwks')) return Response.json({ keys: [jwk] }); return new Response(null, { status: 404 }); });
    const started = await app.request(authRequired.authenticationUrl); expect(started.status).toBe(302); const authorizationUrl = new URL(started.headers.get('location')!); const state = authorizationUrl.searchParams.get('state')!; const nonce = authorizationUrl.searchParams.get('nonce')!; const stateCookie = started.headers.get('set-cookie')!.split(';')[0]!;
    idToken = await new SignJWT({ email: 'investor@example.com', email_verified: true, name: 'Investor', nonce, auth_time: Math.floor(Date.now() / 1000) }).setProtectedHeader({ alg: 'RS256', kid: 'participant-test-key' }).setIssuer(issuer).setAudience(clientId).setSubject('cognito-sub-123').setIssuedAt().setExpirationTime('5m').sign(privateKey);
    const callback = await app.request(`/auth/participant-callback?code=authorization-code&state=${encodeURIComponent(state)}`, { headers: { cookie: stateCookie } }); fetchMock.mockRestore();
    expect(callback.status).toBe(302); expect(callback.headers.get('location')).toBe(`${config.WEB_URL}/invite`); const externalCookie = callback.headers.get('set-cookie')!.split(',').find((item) => item.trim().startsWith('bc_external_session='))!.trim().split(';')[0]!;
    const session = await app.request('/public/session', { headers: { cookie: externalCookie } }); expect(session.status).toBe(200); expect(await session.json()).toMatchObject({ participant: { externalPrincipalId: expect.stringMatching(/^principal_/), email: 'investor@example.com' }, agreement: { integrationContext: { identityIssuer: issuer, externalSubject: 'cognito-sub-123', externalPrincipalId: expect.stringMatching(/^principal_/) } } });
  });
