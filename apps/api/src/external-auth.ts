...
export async function setExternalSession(
  context: Context,
  ...
  return async (context, next) => {
    ...
    if (!token)
      return context.json(
        {
          error: "unauthorized",
          message: "Open a valid invitation to continue.",
        },
        401,
      );
    try {
      ...
      if (payload.kind !== "external-participant")
      ...
      return context.json(
        {
          error: "unauthorized",
          message: "The invitation session has expired.",
        },
        401,
      );
    }
