...
function IntegrationSettings({ entity, canManage, onSaved, onError }: { entity: CustomerEntity; canManage: boolean; onSaved: (entity: CustomerEntity) => void; onError: (message: string) => void }) {
  ...
  return (
    <div className="page">
      ...
      {canManage && <IntegrationClientManager entity={entity} onError={onError} />}
...
function IntegrationClientManager({ entity, onError }: { entity: CustomerEntity; onError: (message: string) => void }) {
  ...
  const create = async (event: FormEvent) => { event.preventDefault(); setBusy('create'); try { const result = await api.createIntegration({ name, key, mappingStrategy: strategy, allowedRedirectUris: [redirectUri], ...(strategy === 'shared_oidc' ? { identityProviderKey: 'participant-oidc' as const } : {}) }); setClients((items) => [...items, result.integration]); setRevealed({ clientId: result.integration.clientId!, clientSecret: result.clientSecret, name: result.integration.name }); setCreating(false); setName(''); setKey(''); setRedirectUri(''); } catch (cause) { onError(cause instanceof Error ? cause.message : 'Could not create API client.'); } finally { setBusy(undefined); } };
  const rotate = async (client: Integration) => { if (!window.confirm(`Rotate the client secret for ${client.name}? The current secret will stop working immediately.`)) return; setBusy(`rotate-${client.key}`); try { const result = await api.rotateIntegrationSecret(client.key); setClients((items) => items.map((item) => item.id === result.integration.id ? result.integration : item)); setRevealed({ clientId: result.integration.clientId!, clientSecret: result.clientSecret, name: result.integration.name }); } catch (cause) { onError(cause instanceof Error ? cause.message : 'Could not rotate the client secret.'); } finally { setBusy(undefined); } };
  return <section className="plugin-manager integration-client-manager">
