...
For the concrete Cognito, Google Drive, entity bootstrap, and single-host Compose profile, see the [FiftySixty deployment guide](./fiftysixty-deployment.md). Its provider-specific values configure adapters at the edge; they do not introduce FiftySixty concepts into the CLM domain.
