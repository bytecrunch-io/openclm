...
const integrationCredentials = await json(await fetch(`${apiUrl}/v1/integrations`, { method: 'POST', headers: adminHeaders, body: JSON.stringify({ key: integrationKey, name: 'Customer portal local E2E', mappingStrategy: 'host_asserted', allowedRedirectUris: ['http://localhost:3000/workflows'], allowedOrigins: ['http://localhost:3000'] }) }));
const machineToken = await json(await fetch(`${apiUrl}/oauth/token`, { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded', authorization: `Basic ${Buffer.from(`${integrationCredentials.integration.clientId}:${integrationCredentials.clientSecret}`).toString('base64')}` }, body: new URLSearchParams({ grant_type: 'client_credentials', scope: 'conditions:read signing_sessions:write' }) }));
const integrationHeaders = { authorization: `Bearer ${machineToken.access_token}`, 'content-type': 'application/json' };
const integrationSession = await json(await fetch(`${apiUrl}/integration/v1/signing-sessions`, { method: 'POST', headers: integrationHeaders, body: JSON.stringify({ integrationKey, subject: `portal-user-${marker}`, email: `visitor-${marker}@example.test`, displayName: 'Portal User', templateKey: 'mutual-nda', returnUrl: 'http://localhost:3000/workflows', metadata: { workflow: `supplier-${marker}` } }) }));
...
const conditionEvaluation = await json(await fetch(`${apiUrl}/integration/v1/conditions/evaluate`, { method: 'POST', headers: integrationHeaders, body: JSON.stringify({ integrationKey, subject: `portal-user-${marker}`, operator: 'all', conditions: [{ kind: 'subject_signed', templateKey: 'mutual-nda', minimumVersion: 1 }, { kind: 'agreement_executed', templateKey: 'mutual-nda', minimumVersion: 1 }] }) }));
